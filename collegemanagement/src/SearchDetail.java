...
class SearchDetail extends DBCON implements  WindowListener,ActionListener
{
...
SearchDetail() throws Exception
{
	...
	try
	{
		...
		ResultSet rs = stmt.executeQuery("select * from admision");
...
{
            try {
                SearchDetail cp = new SearchDetail();
            } catch (Exception ex) {
                Logger.getLogger(SearchDetail.class.getName()).log(Level.SEVERE, null, ex);
            }
}
}